...
async def upsert_chat_info(session: AsyncSession, chat_id: int, **kwargs) -> None:
...
async def get_chat(session: AsyncSession, chat_id: int) -> Chat | None:
    chat = await session.get(Chat, {"chat_id": chat_id})
    return chat


async def get_chats_to_reminder(session: AsyncSession, current_hour: int) -> list[Chat]:
