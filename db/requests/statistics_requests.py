...
async def get_chat_statistic(session: AsyncSession, chat_id: int) -> dict | None:
...
async def change_word_status(session: AsyncSession, chat_id: int, word_id: int, status: str):
    stmt = (
...
async def save_statistic_by_word(
    session: AsyncSession, chat_id: int, word_id: int, correct: int = 0, wrong: int = 0
) -> None:
    stmt = (
        upsert(ChatStatistic)
        .values(
...
async def upsert_chat_event_stat(session: AsyncSession, chat_id: int, date: datetime.date) -> None:
