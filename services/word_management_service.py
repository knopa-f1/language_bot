...
class WordManagementService(BaseService):
    ...
    async def add_current_words(self, chat_id: int, count: int | None = None) -> None:
        if count is None:
    ...
    async def mark_word_as_already_know(self, chat_id: int, word_id: int):
    ...
    async def prepare_words_to_learn(self, chat_id: int, answer_text: str = "") -> dict:
        ...
        if type_id in (1, 2):
            ...
            word = (
                words["correct_word"].word
                if type_id == 1
                else getattr(words["correct_word"], f"translation_{self.lang}")
            )
            answer_id = words["correct_word"].word_id
            msg = self.i18n.get(f"message-text-{type_id}")
            ...
            msg = self.i18n.get(f"message-text-{type_id}")
            translation = getattr(correct_word, f"translation_{self.lang}")
    ...
    async def process_word(self, button_word, statistics_service: StatisticsService):
        ...
        await statistics_service.save_statistic(
            button_word.chat_id,
            button_word.word_id,
        ...
        translation = getattr(correct_word, f"translation_{self.lang}")
        example = getattr(correct_word, f"example_{self.lang}")
        return (
            f"{self.i18n.correct.answer() if button_word.correct else self.i18n.wrong.answer()}\n\n"
            f'{translation if button_word.type_id == "1" else correct_word.word} '
            f'- {correct_word.word if button_word.type_id == "1" else translation}\n\n'
    ...
    async def process_letters(
        self, button_word, statistics_service: StatisticsService
        ...
        if not state or state["word_id"] != word_id:
            return (
                ...
                Keyboards.cancel_learning_keyboard(self.i18n),
            )
