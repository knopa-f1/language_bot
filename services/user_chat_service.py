...
class UserChatService(BaseService):
    async def user_exist(self, user_id: int, chat_id: int) -> int | str | None:
    ...
    async def set_user(self, user: TUser, chat_id: int):
        ...
        await upsert_user(self.session, user.id, chat_id, get_user_info(user))
    ...
    async def set_chat_info(self, chat: TChat, **kwargs) -> None:
        await upsert_chat_info(self.session, chat.id, **kwargs)
