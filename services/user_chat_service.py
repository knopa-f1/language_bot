...
class UserChatService(BaseService):
    ...
    async def user_exists(self, user_id: int, chat_id: int) -> int | str | None:
        user_exists = await self.cache.user_exists(
            user_id,
        )
        if not user_exists:
            ...
            if user is not None:
                user_exists = True
                await self.cache.set_user(user_id, chat_id)
        return user_exists

    async def set_user(self, user: TUser, chat_id: int):
        await self.cache.set_user(user.id, chat_id)
    ...
    async def get_chat_settings(self, chat_id: int, name: str = "lang") -> int | str | None:
        value = await self.cache.get_chat_settings(chat_id, name)
        if value is None:
            chat: Chat | None = await self.chats_repo.get_chat(chat_id)
            if chat is not None:
                value = getattr(chat, name)
                await self.cache.set_chat_settings(chat_id, **{name: value})
        return value

    async def set_chat_settings(self, chat: TChat, **kwargs) -> None:
        await self.cache.set_chat_settings(chat.id, **kwargs)
